...
class GraphEncoder(torch.nn.Module):
    ...
    def forward(self, indices, edge_index):
...
class MultimodalTokenizer(torch.nn.Module):
    def __init__(self, text_model_name: str = "bert-base-uncased", graph_model_name: str = "GCN",
                 ...
                 encoder_text_code_dim: int = 768, decoder_dim: int = 256, encoder_dim: int = 256, output_channels: int = 128,
                 encoder_decoder_hidden_states: int = 768,
        ...
        self.graph_encoder = GraphEncoder(model_name=graph_model_name, in_channels=graph_in_channels, hidden_channels=graph_hidden_channels, out_channels=graph_out_channels, num_nodes=2000000)
    ...
    def tokenize_graph(self, x, edge_index):
        """Encodes the graph input using a graph neural network."""
        return self.graph_encoder(x, edge_index)
    
    def quant(self, text_features, graph_node_features, graph_features, text_features_aug, graph_node_features_aug, graph_features_aug, batch):
        ##input: text_features: [bz, text_dim], graph_features: [bz, graph_dim]
        text_sentence_features = text_features  ##[bz, text_dim]
        text_sentence_features_aug = text_features_aug  ##[bz, text_dim]
        ...
        if self.enable_var:
            ...
            final_quantize = self.quantize(h, text_features, graph_node_features, batch)
            # final_quantize = self.quantize(h, text_features, graph_node_features, batch, h_aug)
    ...
    def forward(self, inputs):
        ...
        graph_edge_index = inputs.edge_index
        graph_edge_index_aug = inputs.edge_index
        batch = inputs.batch
        
        ##graph encoder
        graph_node_features = self.graph_encoder(inputs.n_id, graph_edge_index)[-1] ##[bz, node_num, graph_dim], returned feature is a list recording the hidden states of each layer
        graph_features = graph_node_features[:len(inputs.input_id)] 
        graph_node_features_aug = self.tokenize_graph(inputs.n_id, graph_edge_index_aug)[-1] ##[bz, node_num, graph_dim], returned feature is a list recording the hidden states of each layer
        graph_features_aug = graph_node_features_aug[:len(inputs.input_id)]
        ...
        text_features = inputs.x[:len(inputs.input_id)]
        text_features_aug = inputs.x[:len(inputs.input_id)]
        text_features = self.text_mapped(text_features)

        ##mapped text_features and graph_features to the same codebook
        quantized_result = self.quant(text_features, graph_features, graph_features, text_features_aug, graph_features_aug, graph_features_aug, batch)
