...
parser.add_argument("--num_cpus", type=int, default=32,
                    help='num cpus')
parser.add_argument("--rep_type", type=str, default='description',
...
if not os.path.exists(edge_path):
    ...
    cidx = concepts[['concept_id']].drop_duplicates(keep='first').index
    concepts = concepts.loc[cidx].reset_index(drop=True)
    ...
    edge_counts = mpd.Series(edges.reshape(-1)).value_counts()
    over100 = edge_counts[edge_counts > 100].index.tolist()
    edges_df = mpd.DataFrame(edges.T, columns=['c1', 'c2'])
    edges = edges_df[(~edges_df['c1'].isin(over100)) & (~edges_df['c2'].isin(over100))].values.T
...
for epoch in range(config['num_epochs']):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # KL-loss calculation
        optimizer.zero_grad()
        klloss = model.kl_loss(batch.x, batch.edge_index)
        klloss.backward()
        optimizer.step()
        ...
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [Train batch loss: %.4f] [Train batch kl loss: %.4f] ETA: %s"
            % (epoch+1, config['num_epochs'], i+1, len(train_loader), loss.item(), klloss.item(), time_left)
        )
        epoch_loss += loss.item()
        epoch_klloss += klloss.item()

    epoch_loss = epoch_loss / len(train_loader)
    epoch_klloss = epoch_klloss / len(train_loader)
    logger.info('Epoch [{}/{}], Train Loss: {:.4f} Train KL Loss: {:.4f}'.format(
            epoch+1, config['num_epochs'], epoch_loss, epoch_klloss))
...
np.save(
    os.path.join(rpath, f'concept_representation_{args.rep_type}+gnn.npy'), # description+gnn = medrep
